...
namespace SomtelTechnicalManagmentSystem_STM
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.LoginPath = "/login"; //this will change secure path from /account/login to default path /home/login
                options.AccessDeniedPath = "/denied";
                options.ExpireTimeSpan = TimeSpan.FromMinutes(20);
                options.Events = new CookieAuthenticationEvents()
                {
                    OnSigningIn = async context => //Only when signing will be fired, this will be first
                    {
                        //Before finishing up creating the ticket for the cookie, we will add a role for the user.
                    ...
                    OnSignedIn = async context => //Only when signing, this will fire after cookie is created
                    {
                        await Task.CompletedTask;
                    },
                    OnValidatePrincipal = async context => //Will be fired as long you are signed in
                    {
                        await Task.CompletedTask;
                    },
                };
